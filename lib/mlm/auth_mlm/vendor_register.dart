...
class VendorRegisterState extends State<VendorRegister> {
  ...
  LatLng initPosition = const LatLng(
    0,
    0,
  ); //initial Position cannot assign null values
  LatLng currentLatLng = const LatLng(
    0.0,
    0.0,
  ); //initial currentPosition values cannot assign null values
    ...
    if (value.length == 6) {
      Api.httpWithoutLoader
          .post(
            ...
            queryParameters: {"code": _sponsorIdController.text},
          )
          .then((res) {
            setState(() {
      ...
      child: premiumCard(
        child: Column(
          children: [
            ...
            // MOVE your existing basic fields here
            const SizedBox(height: 20),

            premiumButton("Next", nextStep),
          ],
        ),
      ),
    );
      ...
      child: premiumCard(
        child: Column(
          children: [
              ...
              onChanged: (value) {
                ...
                if (value!.length == 11) {
                  Api.httpWithoutBaseUrl
                      .get(
                        'https://ifsc.razorpay.com/' + _ifscCodeController.text,
                      )
                      .then((res) {
                        setState(() {
            ...
            Row(
              children: [
                Expanded(child: premiumButton("Previous", previousStep)),
                10.width,
                Expanded(child: premiumButton("Next", nextStep)),
              ],
            ),
          ],
        ),
                      ...
                      if (false) {
                        ...
                        Map<String, dynamic> sendData = {
                          ...
                          if (isPetrolPump == false)
                            "vendor_percentage": vendorSelection,
