...
class _AppDrawerState extends State<AppDrawer> {
  final List<Entry> data = <Entry>[
    ...
    if (Auth.isUserVendor!) Entry("Audio Settings", {'icon': UniconsLine.user, 'page': '/audio-settings'}),
    Entry(
      "My Banners",
      {'icon': UniconsLine.money_bill},
      <Entry>[
        Entry("Customer-Customer", {'page': '/customer-to-customer'}),
        if (Auth.user()!['isPromotor'] != null && Auth.user()!['isPromotor'] == true)
          Entry("Promoter-Vendor", {'page': '/customer-to-vendor'}),
        if (Auth.isUserVendor!) Entry("Vendor-Customer", {'page': '/vendor-to-customer'}),
      ],
    ),
    ...
    Entry("Online Purchase Invoice", {'icon': UniconsLine.shopping_cart_alt, 'page': 'orders'}),
    Entry("Payout", {'icon': UniconsLine.briefcase, 'page': '/payout'}),
    if (Auth.user()!['isPromotor'] != null && Auth.user()!['isPromotor'] == true)
      Entry("Promoter Payout", {'icon': UniconsLine.briefcase, 'page': '/promoter-payout'}),
    if (Auth.isVendor()!) ...[
      ...
      Entry("Vendor Wallet Transaction", {'icon': UniconsLine.wallet, 'page': '/vendor-wallet-transaction'}),
    ],
    Entry("Offline Order", {'icon': UniconsLine.briefcase, 'page': '/off-line-orders'}),
    Entry("Offline Shop Complaints", {'icon': UniconsLine.headphones_alt, 'page': '/help-support-list'}),
    // Entry("Withdrawal Request", {'icon': UniconsLine.web_grid_alt, 'page': '/withdrawal-request-list'}),
    Entry("Wallet Transactions", {'icon': UniconsLine.wallet, 'page': 'wallet'}),
    Entry("Coin Wallet Transactions", {'icon': UniconsLine.wallet, 'page': 'coin-wallet'}),
    if (Auth.user()!['isPromotor'] != null && Auth.user()!['isPromotor'] == true) ...[
      Entry("Pending Wallet Transactions", {'icon': UniconsLine.wallet, 'page': 'pending-wallet'}),
      Entry("Promoter Wallet Transactions", {'icon': UniconsLine.wallet, 'page': 'promoter-wallet'}),
    ],
    Entry("Near By Offline Store", {'icon': UniconsLine.store, 'page': 'nearby-offline-store'}),
    Entry("Reports", {'icon': UniconsLine.file, 'page': 'reports'}),
    Entry("Introductory Video", {'icon': UniconsLine.video, 'page': '/introductory-video'}),
    Entry("Self Explanatory Video", {'icon': UniconsLine.video, 'page': '/self-explanatory-video'}),
...
class _EntryItemState extends State<EntryItem> {
  ...
  Widget _buildTiles(Entry root, {int depth = 0}) {
    if (root.children.isEmpty) {
      return Padding(
        ...
        child: ListTile(
          leading: Icon(root.data['icon'], color: colorPrimary),
          onTap: () {
            Get.back();
            setState(() {
              _expanded = false;
  ...
  @override
  Widget build(BuildContext context) {
    return Container(child: _buildTiles(widget.entry));
  }
}
